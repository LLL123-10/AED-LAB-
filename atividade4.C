...
void curarTodos(Inimigo *vetor, int n, int cura) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Aplica a cura e garante que não ultrapasse a vida máxima
        ini->vida += cura;
        if (ini->vida > vidaMaxima) {
            ini->vida = vidaMaxima;
        }
    }
}
...
Inimigo *encontrarInimigoMaisFraco(Inimigo *vetor, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Se for o primeiro vivo encontrado ou tiver menos vida que o atual mais fraco
        if (maisFraco == NULL || ini->vida < maisFraco->vida) {
            maisFraco = ini;
        }
    }
    
    return maisFraco; // Retorna o ponteiro do mais fraco ou NULL se todos estiverem mortos
}
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        // Altera a busca do alvo para focar no inimigo com menor vida
        Inimigo *alvo = encontrarInimigoMaisFraco(inimigos, TOTAL_INIMIGOS);
        atingirInimigo(alvo, DANO_TIRO);
        }

        if (IsKeyPressed(KEY_C)) {
        curarTodos(inimigos, TOTAL_INIMIGOS, 15); // Cura todos os vivos em 15 pontos
        }
