...
typedef struct {
    ...
    int       valor;
    bool      coletada;
    float tempoColeta; // Novo campo adicionado
} Moeda;
...
Color corDaMoeda(TipoMoeda tipo) {
    switch (tipo) {
        ...
        case MOEDA_DIAMANTE: return SKYBLUE; // Cor azul celeste para o diamante
...
Moeda *criarMoedas(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        // Sorteia um número de 0 a 9. Se for 0 (10% de chance), vira diamante
...
bool tentarColetar(Moeda *m, Vector2 posJogador, float raioJogador) {
    ...
    if (distancia <= somaRaios) {
        m->coletada = true;
        m->tempoColeta = GetTime(); // Registra o instante da coleta
        return true;
    }
    return false;
}
...
int main(void) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        // Se a moeda foi coletada e já se passaram 3 segundos
        if (m->coletada && (tempoAtual - m->tempoColeta >= 3.0f)) {
            // Sorteia uma nova posição na tela
            m->pos = (Vector2){ 
                GetRandomValue(30, LARGURA_JANELA - 30), 
                GetRandomValue(30, ALTURA_JANELA - 30) 
            };
            m->coletada = false; // Faz a moeda reaparecer
        }
    }
}
