...
int main(void) {
    ...
    void atualizarSpawnerItens() {
    if (IsKeyPressed(KEY_N)) {
        
        if (totalEntidades < MAX_ENTIDADES) {
            
            Vector2 posicaoAleatoria = {
                (float)GetRandomValue(10, 790), // Exemplo para tela de 800 de largura
                (float)GetRandomValue(10, 590)  // Exemplo para tela de 600 de altura
            };

            Entidade *novoItem = criarEntidade(ENTIDADE_ITEM, posicaoAleatoria);

            if (novoItem != NULL) {
                vetorEntidades[totalEntidades] = novoItem;
                totalEntidades++;
            }
        }
    }
...
void ordenarEntidadesPorDistancia() {
    ...
    for (int i = 1; i < totalEntidades - 1; i++) {
        int menorIndice = i;
        float distMenor = Vector2Distance(jogador->pos, vetorEntidades[i]->pos);

        for (int j = i + 1; j < totalEntidades; j++) {
            float distAtual = Vector2Distance(jogador->pos, vetorEntidades[j]->pos);
        ...
        if (menorIndice != i) {
            ...
            vetorEntidades[i] = vetorEntidades[menorIndice];
