...
Item *criarItens(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        it->raio     = 12.0f;
        it->coletado = false;
        it->tipo     = (TipoItem)GetRandomValue(ITEM_ARMA, ITEM_POCAO, ITEM_ESCUDO);
        ...
        if (it->tipo == ITEM_ARMA) {
            ...
            it->dados.arma.alcance = GetRandomValue(1, 3);
             ...
             // Sorteia um número de 0 a 9 para a chance de veneno (30% de chance)
    int chanceVeneno = GetRandomValue(0, 9);
    int valorCura = GetRandomValue(10, 30);
    
    if (chanceVeneno < 3) {
        it -> dados . pocao . cura = -valorCura; // Cura negativa (veneno)
    } else {
...
void desenharItem(Item *item) {
    ...
    Color cor = (item->tipo == ITEM_ARMA) ? RED : GREEN : BLUE;
    DrawCircleV(item->pos, item->raio, cor);

    switch (item->tipo) {
        case ITEM_ARMA:
            DrawCircleV(item->pos, item->raio, RED);
            break;
        case ITEM_POCAO:
            // Verifica se a poção é envenenada com base no sinal do valor de cura
    if (item->dados.pocao.cura < 0) {
        DrawCircleV(item->pos, item->raio, PURPLE); // Roxo para veneno
    } else {
        DrawCircleV(item->pos, item->raio, GREEN);  // Verde para cura normal
    }
            break;
        case ITEM_ESCUDO:
            // Desenha o escudo com uma cor distinta (ex: azul)
            DrawCircleV(item->pos, item->raio, BLUE);
            break;
    }
}
